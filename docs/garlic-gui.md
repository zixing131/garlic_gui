...
0.4 配套引擎的丰富索引和源码位置映射，不能换成未经修改的上游 garlic。
...
提供 Streamable HTTP 的 POST / JSON 响应，GET 返回 405。设置页配置包含当前进程生成的 Bearer token；
重启 GUI 后需更新 HTTP 配置。拒绝无效 Host / Origin 和未授权请求。两种传输共享当前 GUI 的工具与项目。
stdio 输出仅有 JSON-RPC，
