...
bool Project::writeIndexSnapshot(QIODevice *device) const {
    ...
    if (!block([&](QDataStream &out) {
        out << input_ << inputs_ << classNames_ << parents_ << aliases_ << aliasVersion();
    ...
    out << QByteArray("GARLIC-INDEX-4") << quint32(blocks.size());
...
std::shared_ptr<Project> Project::readIndexSnapshot(QIODevice *device, const std::shared_ptr<std::atomic_bool> &canceled) {
    ...
    if (magic != "GARLIC-INDEX-4" || count != Shards * 2 + 1) return {};
    ...
    for (int task = 0; task < blocks.size(); ++task) {
        ...
        if (b < Shards * 2) {
            ...
            in >> project->input_ >> project->inputs_ >> project->classNames_ >> project->parents_ >> project->aliases_ >> project->aliasIndex_->version;
