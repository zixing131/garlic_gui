...
private slots:
    void safeNames() {
        ...
        for (const QString &name : {"../evil", "/absolute", "a//b", "a/../b", "C:/file", "a\\b", "", "a\nb"})
    ...
    void realFiles() {
        QFETCH(QString, file); QFETCH(bool, smali);
        ...
        QVERIFY(!names.contains("demo/Main$Details"));
        backend.request("demo/Main", false);
        QTRY_COMPARE_WITH_TIMEOUT(source.count(), 1, 15000);
        QFile code(source.first().at(2).toString()); QVERIFY(code.open(QIODevice::ReadOnly));
        const auto text = code.readAll(); QVERIFY(text.contains("class Main")); QVERIFY(text.contains("greet"));
        if (file != "Main.class") QVERIFY(text.contains("Details"));
        ...
        QCOMPARE(source.count(), 2); QVERIFY(!backend.busy());
        ...
        if (smali) {
            ...
            QFile assembly(source.last().at(2).toString()); QVERIFY(assembly.open(QIODevice::ReadOnly));
        ...
        QVERIFY(QFileInfo::exists(output.path() + (file == "Main.class" ? "/export/source.java" : "/export/demo/Main.java")));
        if (file != "Main.class") QVERIFY(QFileInfo::exists(output.path() + "/export/demo/Extra.java"));
    ...
    void cancelCrashAndBadIndex() {
        Backend backend; backend.setEngine(qEnvironmentVariable("GARLIC_TEST_FAKE_ENGINE"));
        QTemporaryDir files;
        for (const QString &name : {"slow.jar", "crash.jar", "bad.jar"}) {
            QFile file(files.filePath(name)); QVERIFY(file.open(QIODevice::WriteOnly)); file.write("test");
        }
        QSignalSpy errors(&backend, &Backend::failed);
        backend.open(files.filePath("slow.jar")); QVERIFY(backend.busy());
        QTest::qWait(100); backend.cancel();
        QTRY_VERIFY_WITH_TIMEOUT(!backend.busy(), 5000); QVERIFY(errors.isEmpty());
        backend.open(files.filePath("crash.jar"));
        QTRY_COMPARE_WITH_TIMEOUT(errors.count(), 1, 5000); QVERIFY(!backend.busy());
        backend.open(files.filePath("bad.jar"));
        QTRY_COMPARE_WITH_TIMEOUT(errors.count(), 2, 5000); QVERIFY(!backend.busy());
    }
};
