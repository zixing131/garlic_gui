...
public class Check {
    ...
    subprocess.run([str(engine), str(fixtures / 'demo.jar'), '-o', str(flattened), '-c', 'demo/Folded', '-t', '1'],
                   check=True, capture_output=True, timeout=20, env=env)
    ...
    subprocess.run([str(engine), str(fixtures / 'demo.jar'), '-o', str(disabled), '-c', 'demo/Folded', '-t', '1'],
...
public class Check {
    ...
    subprocess.run(['javac', '-d', str(flattened), str(folded), str(runner)], check=True,
                   capture_output=True, timeout=30)
    subprocess.run(['java', '-cp', str(flattened), 'Check'], check=True,
                   capture_output=True, timeout=20)
